(() => {
    ...
    const STORAGE_KEY = "guideCampPlanner.android.project.v1";
    const DRAFT_KEY = "guideCampPlanner.android.draftName.v1";
    ...
    const COLLAB_AAD = "GuideCampPlanner.FirebaseCollaboration.v1";
    ...
    const TERMS = {
        ...
        camperTypeRainbow: "Rainbow",
        camperTypeBrownie: "Brownie",
        camperTypeGuide: "Guide",
        camperTypeRanger: "Ranger",
    ...
    const BUDGET_CAMPER_TYPES = [TERMS.camperTypeRainbow, TERMS.camperTypeBrownie, TERMS.camperTypeGuide, TERMS.camperTypeRanger, TERMS.camperTypeStandard];
    ...
    function normalizeProject(project) {
        ...
        project.campName = clean(project.campName, "New Guide camp");
    ...
    function orderedProjectPeople(project) {
        const group = personItem => {
            if (personItem.personType === TERMS.personTypeYoungPerson) {
                return ["Rainbow", "Brownie", "Guide", "Ranger", "Standard"].indexOf(personItem.camperType) + 1 || 5;
            }
    ...
    function person(data = {}) {
        return {
            ...
            gender: data.gender || TERMS.genderFemale,
    ...
    function mapGender(value) {
        ...
        return TERMS.genderFemale;
    }

    function mapCamperType(value) {
        const text = clean(value).toLowerCase();
        if (text.includes("squirrel") || text === "0") return TERMS.camperTypeRainbow;
        if (text.includes("beaver") || text === "1") return TERMS.camperTypeRainbow;
        if (text.includes("cub") || text === "2") return TERMS.camperTypeBrownie;
        if (text.includes("scout") || text === "3") return TERMS.camperTypeGuide;
        if (text.includes("explorer") || text === "4") return TERMS.camperTypeRanger;
        if (text.includes("rainbow")) return TERMS.camperTypeRainbow;
        if (text.includes("brownie")) return TERMS.camperTypeBrownie;
        if (text.includes("guide")) return TERMS.camperTypeGuide;
        if (text.includes("ranger")) return TERMS.camperTypeRanger;
    ...
    function saveDraft() {
        try {
            ...
            localStorage.setItem(DRAFT_KEY, State.fileName || "camp.guidecamp");
    ...
    function orderedPeople() {
        const group = person => {
            if (person.personType === TERMS.personTypeYoungPerson) {
                return ["Rainbow", "Brownie", "Guide", "Ranger", "Standard"].indexOf(person.camperType) + 1 || 5;
            }
    ...
    function init() {
        ...
        State.fileName = draftName || `${safeFileName(State.project.campName)}.guidecamp`;
        bindGlobalEvents();
    ...
    function renderCamperTypeMark(camperType) {
        if (camperType === TERMS.camperTypeRainbow) {
            return `
                <g transform="translate(9 12) scale(0.6)">
                    <path d="M 1,34 A 20,20 0 0 1 41,34" fill="none" stroke="#d32f2f" stroke-width="3"></path>
                    <path d="M 4,34 A 17,17 0 0 1 38,34" fill="none" stroke="#ff9800" stroke-width="3"></path>
                    <path d="M 7,34 A 14,14 0 0 1 35,34" fill="none" stroke="#ffd500" stroke-width="3"></path>
                    <path d="M 10,34 A 11,11 0 0 1 32,34" fill="none" stroke="#43a047" stroke-width="3"></path>
                    <path d="M 13,34 A 8,8 0 0 1 29,34" fill="none" stroke="#1e88e5" stroke-width="3"></path>
                    <path d="M 16,34 A 5,5 0 0 1 26,34" fill="none" stroke="#7e57c2" stroke-width="3"></path>
                    <circle cx="2" cy="33" r="4.5" fill="#fff" stroke="#cdd6e3" stroke-width="0.6"></circle>
                    <circle cx="8" cy="35" r="5" fill="#fff" stroke="#cdd6e3" stroke-width="0.6"></circle>
                    <circle cx="40" cy="33" r="4.5" fill="#fff" stroke="#cdd6e3" stroke-width="0.6"></circle>
                    <circle cx="34" cy="35" r="5" fill="#fff" stroke="#cdd6e3" stroke-width="0.6"></circle>
                </g>`;
        }
        if (camperType === TERMS.camperTypeBrownie) {
            return `
                <g transform="translate(12 9) scale(0.68)">
                    <path d="M10,17 C3,13 1,6 6,4 C11,2 14,10 13,19 Z" fill="rgba(255,255,255,0.85)" stroke="#cdd6e3" stroke-width="0.8"></path>
                    <path d="M23,17 C30,13 32,6 27,4 C22,2 19,10 20,19 Z" fill="rgba(255,255,255,0.85)" stroke="#cdd6e3" stroke-width="0.8"></path>
                    <line x1="28" y1="27" x2="32" y2="18" stroke="#a98200" stroke-width="1.2"></line>
                    <circle cx="31.2" cy="18" r="1.8" fill="#ffd54f" stroke="#a98200" stroke-width="0.5"></circle>
                    <path d="M17,15 C12,15 8,23 7,30 C13,33 21,33 27,30 C26,23 22,15 17,15 Z" fill="#ffc425" stroke="#a98200" stroke-width="1.1"></path>
                    <polygon points="17,1 11,9 23,9" fill="#ffc425" stroke="#a98200" stroke-width="0.8"></polygon>
                    <circle cx="17" cy="13" r="6" fill="#ffd6aa" stroke="#a98200" stroke-width="0.8"></circle>
                </g>`;
        }
        if (camperType === TERMS.camperTypeRanger) {
            return `
        ...
        if (camperType === TERMS.camperTypeGuide) {
            return `
                <g transform="translate(9 9) scale(0.0432)" fill="#1565c0" stroke="#0d47a1" stroke-width="6">
                    <polygon points="12,434 5,501 15,549 43,602 87,645 150,674 216,679 262,669 314,641 354,600 380,551 390,504 387,457 197,457 198,523 321,525 299,566 267,595 228,612 184,615 133,598 90,557 71,509 74,451 88,420 116,388 171,361 239,365 298,405 346,357 313,309 299,262 301,206 316,165 339,132 383,97 426,81 476,78 518,88 556,109 584,135 610,177 622,223 620,271 606,312 579,353 624,404 650,380 686,363 756,363 814,398 847,459 845,522 819,570 772,605 708,615 670,605 642,588 615,559 599,524 720,523 721,457 531,457 529,506 539,551 560,593 593,631 626,655 682,676 739,679 791,667 841,638 882,594 906,543 914,497 902,419 877,373 851,344 811,316 766,299 679,297 686,263 682,192 662,136 634,94 564,38 482,14 390,24 311,69 269,118 244,171 234,225 241,298 169,296 104,318 41,374"></polygon>
                    <polygon points="428,457 427,762 491,762 492,459"></polygon>
                    <polygon points="460,159 441,215 382,217 429,252 412,308 459,274 509,308 491,252 539,216 479,215"></polygon>
                </g>`;
        }
        return "";
    }
    ...
    function personBadge(person) {
        ...
        if (person.camperType === TERMS.camperTypeRainbow) return { label: "Rainbow", background: "#009fda", foreground: "#ffffff" };
        if (person.camperType === TERMS.camperTypeBrownie) return { label: "Brownie", background: "#ffc425", foreground: "#000000" };
        if (person.camperType === TERMS.camperTypeGuide) return { label: "Guide", background: "#1565c0", foreground: "#ffffff" };
        if (person.camperType === TERMS.camperTypeRanger) return { label: "Ranger", background: "#5a2c82", foreground: "#ffffff" };
        return { label: "Camper", background: "#1565c0", foreground: "#ffffff" };
    }
        ...
        return `
            ...
            <div class="card" style="margin-bottom:12px">
                <label style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
                    <span style="font-weight:800;color:var(--blue-900)">PDF page size</span>
    ...
    async function openProject() {
        ...
        if (!project || !Array.isArray(project.people)) {
            throw new Error("The selected file does not contain a Guide camp project.");
        }
        State.project = normalizeProject(project);
        State.fileName = file.name || `${safeFileName(State.project.campName)}.guidecamp`;
        State.dirty = false;
        State.undo = [];
        State.redo = [];
        saveDraft();
    ...
    async function saveProject() {
        normalizeProject(State.project);
        State.project.lastModified = new Date().toISOString();
        const fileName = State.fileName && State.fileName.endsWith(".guidecamp") ? State.fileName : `${safeFileName(State.project.campName)}.guidecamp`;
    ...
    async function downloadPeopleSampleCsv() {
        const rows = [
            ...
            "Alex Green,Camper,Brownie,No,Other,Vegetarian,Inhaler,",
    ...
    function buildSectionFile(sectionKey) {
        ...
        return { type: "guide-camp-section", version: 1, sectionKey, sectionLabel: SECTION_TITLES[sectionKey], data };
    }

    async function saveSection(sectionKey) {
        const sectionFile = buildSectionFile(sectionKey);
        await saveTextFile(`${safeFileName(State.project.campName)}-${sectionKey}.guidesection`, "application/json", JSON.stringify(sectionFile, null, 2));
    }

    async function importSection(sectionKey) {
        const file = await requestFile();
        const sectionFile = JSON.parse(file.text);
        if (sectionFile.type !== "guide-camp-section" || sectionFile.sectionKey !== sectionKey) {
    ...
    async function exportMenuPdf() {
        const pdf = new GuidePdf("Camp menu", State.project, { toc: false });
        const warnings = buildMenuWarnings(State.project);
    ...
    async function exportKitchenMenuPdf() {
        const pdf = new GuidePdf("Kitchen menu", State.project, { toc: false });
    ...
    async function exportChoresPdf() {
        const pdf = new GuidePdf("Chore rota", State.project, { toc: false });
    ...
    async function exportPlanPdf() {
        const pdf = new GuidePdf("The Plan", State.project, { toc: false });
    ...
    async function exportShoppingPdf() {
        const pdf = new GuidePdf("Shopping lists", State.project, { toc: false });
    ...
    async function exportTentAllocationRtf() {
        ...
        const content = [
            ...
            "{\\colortbl;\\red0\\green0\\blue0;\\red21\\green101\\blue192;\\red255\\green255\\blue255;\\red227\\green242\\blue253;\\red190\\green210\\blue230;\\red184\\green45\\blue45;}",
    ...
    function buildTentAllocationXlsx(project, rows) {
        ...
        const files = [
            ...
            ["docProps/app.xml", `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"><Application>Guide Camp Planner</Application></Properties>`],
            ["docProps/core.xml", `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>${xml(project.campName)} - Tent allocation</dc:title><dc:creator>Guide Camp Planner</dc:creator><dcterms:created xsi:type="dcterms:W3CDTF">${new Date().toISOString().replace(/\.\d{3}Z$/, "Z")}</dcterms:created></cp:coreProperties>`],
    ...
    async function exportTentTablePdf() {
        const pdf = new GuidePdf("Tent allocation table", State.project, { orientation: "landscape", toc: false });
    ...
    async function exportTentTagsPdf() {
        const pdf = new GuidePdf("Tent tags", State.project, { plain: true, toc: false });
    ...
    async function exportTentLayoutPdf() {
        const pdf = new GuidePdf("Tent layout", State.project, { toc: false });
    ...
    function addKitchenDetailBlock(pdf, slot, item) {
        ...
        const y = pdf.y;
        pdf._line(pdf.ML, y, pdf.W - pdf.MR, y, "blue-light", 0.7);
        pdf._text(slot, pdf.ML, y + 16, 9, true, "blue-dark");
        let ty = y + 14;
    ...
    async function savePdf(fileName, title, lines) {
        const pdf = new GuidePdf(title, State.project);
        lines.forEach(line => {
            if (line.pageBreak) {
                pdf._addNewPage();
    ...
    class GuidePdf {
        ...
        _drawCoverBand() {
            ...
            this._rect(0, 0, this.W, 54, "blue-dark", 0);
            ...
            this._rect(0, 54, this.W, 3, "blue-light", 0);
        }
        ...
        _addNewPage() {
            ...
            this._rect(0, this.H - 30, this.W, 30, "blue-dark");
            this._text("Guide Camp Planner", this.ML, this.H - 14, 8, false, "white");
        ...
        _drawCoverBandSmall() {
            ...
            this._rect(0, 0, this.W, 26, "blue-dark");
        ...
        addSectionBanner(text) {
            ...
            this._rect(this.ML - 8, this.y - 14, this.W - this.ML - this.MR + 16, 22, "blue-mid");
            this._text(text, this.ML, this.y, 11, true, "white");
            this.y += 14;
            this._rect(this.ML - 8, this.y, this.W - this.ML - this.MR + 16, 1, "blue-light");
            this.y += 9;
        }
        ...
        addSubHeading(text, colorHint) {
            this._needSpace(20);
            this.y += 4;
            const col = colorHint === "red" ? "red" : "blue-dark";
        ...
        addTable(columns, rows, options = {}) {
            ...
            const drawHeader = () => {
                this._rect(this.ML, this.y, totalWidth, headerHeight, "blue-mid");
                let x = this.ML;
        ...
        _drawColumnSection(x, y, width, height, section) {
            ...
            this._text(section.title, x + 7, y + 15.5, 9.5, true, "blue-dark");
            let textY = y + 36;
            const lines = (section.lines || []).filter(line => clean(line)).length ? section.lines : ["No items."];
            lines.forEach(line => {
        ...
        insertTableOfContents() {
            ...
            const tocLines = [];
            let y = 80;
            tocRects.push({ x: 0, y: 0, w: this.W, h: 54, colorKey: "blue-dark" });
            ...
            tocRects.push({ x: 0, y: 54, w: this.W, h: 3, colorKey: "blue-light" });
            this.tocEntries.forEach(entry => {
                ...
                tocPageObjs.push({ x: this.W - this.MR - 30, y, text: pdfEscape(String(entry.page + 2)), size: 11, bold: true, colorKey: "blue-dark" });
        ...
        bytes() {
            ...
            const pdfColorStr = (key, stroke) => {
                ...
                if (key === "blue-dark")  return `0.051 0.278 0.631 ${suffix}`;
                if (key === "blue-mid")   return `0.118 0.533 0.898 ${suffix}`;
                if (key === "blue-light") return `0.392 0.710 0.965 ${suffix}`;
                if (key === "white")       return `1 1 1 ${suffix}`;
                if (key === "white-fill")  return `0.97 0.98 1 ${suffix}`;
                ...
                if (key === "line-dark")   return `0.08 0.20 0.35 ${suffix}`;
                if (key === "row-stripe")  return `0.933 0.953 0.980 ${suffix}`;
    ...
    function pdfColor(color) {
        if (color === "blue") return "0.082 0.396 0.753 rg";
    ...
    function buildShoppingRtf() {
        ...
        const header = `{\\rtf1\\ansi\\deff0{\\fonttbl{\\f0 Segoe UI;}}{\\colortbl;\\red0\\green0\\blue0;\\red21\\green101\\blue192;\\red184\\green45\\blue45;\\red255\\green255\\blue255;}\\paperw11907\\paperh16840\\margl900\\margr900\\margt760\\margb760\n\\pard\\qc\\b\\fs36\\cf2 ${rtfEscape(title)}\\b0\\par\\par\n`;
    ...
    function buildRtf(title, lines) {
        ...
        return `{\\rtf1\\ansi\\deff0{\\fonttbl{\\f0 Segoe UI;}}{\\colortbl;\\red0\\green0\\blue0;\\red21\\green101\\blue192;\\red184\\green45\\blue45;}\\paperw11907\\paperh16840\\margl900\\margr900\\margt760\\margb760\n\\pard\\qc\\b\\fs36\\cf2 ${rtfEscape(title)}\\b0\\par\n${flatLines.map(line => `\\pard\\ql\\fs${line.size && line.size >= 15 ? 26 : 18}${line.bold ? "\\b" : ""}\\cf${line.color === "red" ? 3 : line.color === "blue" ? 2 : 1} ${rtfEscape(line.text)}${line.bold ? "\\b0" : ""}\\par`).join("\n")}\n}`;
    }
        ...
        lines.forEach(line => {
            if (line.twoColumnSections) {
                (line.sections || []).forEach(section => {
                    flat.push({ text: section.title, bold: true, color: "blue" });
    ...
    async function exportCsvZip() {
        ...
        const readme = [
            `Guide Camp Planner — CSV data export`,
    ...
    function about() {
        const body = document.createElement("div");
        body.innerHTML = `
            <p><strong>Guide Camp Planner / Camp Planner</strong></p>
            <p>Version 060626.</p>
        ...
        showModal("About Guide Camp Planner", body, [{ label: "Close", value: "close", className: "secondary" }]);
    }
